...
export async function GET(request: Request) {
  ...
  const month = (d: Date) => new Intl.DateTimeFormat("en-CA", { timeZone: "America/Argentina/Buenos_Aires", year: "numeric", month: "2-digit" }).format(d).slice(0, 7);
  ...
  for (let cursor = new Date(from.getFullYear(), from.getMonth(), 1); cursor <= end; cursor.setMonth(cursor.getMonth() + 1)) {
    const key = `${cursor.getFullYear()}-${String(cursor.getMonth() + 1).padStart(2, "0")}`;
    if (!monthlyMap.has(key)) monthlyMap.set(key, { month: key, label: monthLabel(key), presupuestado: 0, facturado: 0, cobrado: 0 });
  }
  ...
  const recentQuotes = await prisma.quote.findMany({ where: { estado: { not: "ANULADO" } }, select: { id: true, numero: true, obra: true, total: true, estado: true, createdAt: true, client: { select: { razonSocial: true } } }, orderBy: { createdAt: "desc" }, take: 5 });
  const activeWorkOrders = await prisma.workOrder.findMany({ where: { estadoProductivo: { in: ["PENDIENTE", "EN_PROCESO"] } }, select: { id: true, numero: true, obra: true, categoria: true, estadoProductivo: true, porcentajeAvance: true, fechaEntrega: true }, orderBy: { fechaEntrega: "asc" }, take: 5 });
  const totalM2 = active.reduce((sum, workOrder) => sum + workOrder.items.reduce((subtotal, item) => subtotal + n(item.m2), 0), 0);
  return NextResponse.json({ data: { filters: { from: from.toISOString().slice(0, 10), to: end.toISOString().slice(0, 10), tipo }, kpis: { presupuestado: quotes.reduce((s, q) => s + n(q.total), 0), facturado: invoices.reduce((s, i) => s + n(i.total), 0), cobrado: payments.reduce((s, p) => s + paymentValue(p), 0), pendienteCobro: Math.max(0, invoices.reduce((s, i) => s + n(i.total), 0) - paid), presupuestos: quotes.length, otsActivas: active.length, m2EnProduccion: Number(totalM2.toFixed(2)), otAtrasadas: active.filter((w) => w.fechaEntrega < now).length }, monthly: [...monthlyMap.values()].sort((a, b) => a.month.localeCompare(b.month)).map((row) => ({ ...row, presupuestado: Number(row.presupuestado.toFixed(2)), facturado: Number(row.facturado.toFixed(2)), cobrado: Number(row.cobrado.toFixed(2)) })), production: Object.entries(production).map(([status, count]) => ({ status, label: labels[status] || status, count })), paymentsByMethod: Object.entries(methods).map(([method, value]) => ({ method, label: methodLabels[method] || method, amount: Number(value.toFixed(2)) })).sort((a, b) => b.amount - a.amount), categoryMix: Object.entries(categories).map(([category, m2]) => ({ category, m2: Number(m2.toFixed(2)) })), quoteStatus: Object.entries(quotes.reduce((m, q) => { m[q.estado] = (m[q.estado] || 0) + 1; return m; }, {} as Record<string, number>)).map(([status, count]) => ({ status, count })), recentQuotes, activeWorkOrders } });
}

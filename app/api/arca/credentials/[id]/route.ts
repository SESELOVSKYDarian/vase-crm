...
import { arcaConnectionReset, arcaCredentialReset, invalidateArcaTickets } from "@/lib/arca/configuration";
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.$transaction(async (tx) => { await tx.arcaCredentialFile.delete({ where: { id } }); if (file.active) { const settings = await tx.companySettings.findFirst(); if (settings) await tx.companySettings.update({ where: { id: settings.id }, data: { ...arcaCredentialReset, ...arcaConnectionReset } }); await invalidateArcaTickets(tx); } });

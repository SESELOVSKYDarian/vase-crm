...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const order = await tx.workOrder.create({ data: { numero: `OT-${String((await tx.workOrder.count()) + 2001).padStart(4, "0")}`, quoteId: quote.id, clientId: quote.clientId, obra: quote.obra, categoria: quote.tipo === "DVH" ? "DVH" : "SIMPLE", fechaEntrega: quote.fechaEntrega, items: { create: quote.items.map((item) => ({ productoNombre: item.productoNombre, cantidad: item.cantidad, anchoMm: item.anchoMm, altoMm: item.altoMm, m2: Number(item.anchoMm * item.altoMm * item.cantidad) / 1_000_000 })) } } });
